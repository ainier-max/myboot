package cbc.boot.myboot.controller.screen;
...
public class GetSqlData {
    ...
    @PostMapping("/screen/getsqldata.cbc")
    public List<Object> getsqldata(@RequestBody String param, HttpServletRequest request, HttpServletResponse response) {
        /**
         * param:{
         *      db:'db1',
         *      sql:'select username,password from t_user',
         * }
         */
        System.out.println("----------Start(Author:陈斌才)----------");
        System.out.println("执行大屏select操作!");
        ...
        try {
            ...
            String sql22=(String) map.get("sql");
            System.out.println("执行语句："+sql22);
            List<Map<String, Object>> list=sqlSession.selectList("get_sql_data.getSqlData",map);
