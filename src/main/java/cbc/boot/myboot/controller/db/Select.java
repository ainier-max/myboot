...
public class Select {
    ...
    @PostMapping("/cbc/selectByBatch.cbc")
    public List<Object>  selectByBatch(@RequestBody String param, HttpServletRequest request, HttpServletResponse response) {
        System.out.println("----------Start(Author:陈斌才)----------");
        System.out.println("执行select操作!");
        System.out.println("传入参数:");
        System.out.println(param);
        ...
        try {
            ...
            List<Map<String, Object>>  batchSqlList=(ArrayList) map.get("batchSql");
            System.out.println("请求参数1：" + (ArrayList) map.get("batchSql"));

            for(int i=0;i<batchSqlList.size();i++){
                String sql=(String) batchSqlList.get(i).get("sql");
                String resultKey=(String) batchSqlList.get(i).get("resultKey");
                List<Map<String, Object>> list=sqlSession.selectList(sql, batchSqlList.get(i));
                hashMap.put(resultKey, list);
            }
