package cbc.boot.myboot.down.map.tengxunyx;
 ...
 */
class TengXunYXTask implements Runnable {
    String link;
    int i;  //x坐标
    int j;  //y坐标
    int z;  //缩放级别
    ...
    public TengXunYXTask(String link, int i, int j, int z) {
        this.link = link;
        this.i = i;
        this.j = j;
        this.z = z;
    ...
    public static void main(String[] args)
            throws Exception {
        //腾讯矢量地图瓦片下载地址
        String link = "https://p3.map.gtimg.com/sateTiles/{z}/{x16}/{y16}/{x}_{y}.jpg?version=231";
        //max.y=2的n次方减1，除0之外
        ...
        for (int z = 10; z < 15; z++) {
            ...
            for (int i = xmin; i <= xmax; i++) {   //循环X
                for (int j = ymin; j <= ymax; j++) {    //循环Y
                    threadPoolExecutor.execute(new TengXunYXTask(link, i, j, z));
                    //new Thread(new TengXunYXTask(link,i,j,z)).start();    //此种方法会一直创建线程导致死机
                }    //循环Y结束
            }   //循环X结束
    ...
    public void run() {
        try {
            int x16=(int)Math.floor(i /16);
            int y16=(int)Math.floor(j /16);
            String newLinkUrl=link.replace("{x}", i + "").replace("{y}", j + "").replace("{z}", z + "").replace("{x16}", x16 + "").replace("{y16}", y16 + "");
            //System.out.println("newLinkUrl:"+newLinkUrl);
            URL url = new URL(newLinkUrl);
